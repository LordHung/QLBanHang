...
namespace QLBanHang.Models
{
    class mdlNhaCungCap
    {
        ...
        public DataTable GetData()
        {
            ...
            _conn.CMD.CommandText = "select id,CodeNhaCungCap,TenNhaCungCap,CongNoDauKy,SoDienThoai,DiaChi from [tbNhaCungCap]";
        ...
        {
            DataTable table = new DataTable();//Create template table to get data from database
            _conn.CMD.CommandText = String.Format("select ISNULL(id,0) as id from tbNhaCungCap where [tbNhaCungCap].[CodeNhaCungCap] = '{0}'", maNhaCungCap);
            _conn.FillData(table);
        ...
        public string GetMaNhaCungCapById(int idNhaCungCap)
        {
            DataTable table = new DataTable();//Create template table to get data from database
            _conn.CMD.CommandText = String.Format("select CodeNhaCungCap from tbNhaCungCap where id = '{0}'", idNhaCungCap);
            _conn.FillData(table);
            return table.Rows[0]["CodeNhaCungCap"].ToString();
        }
        ...
        {
            _conn.CMD.CommandText = String.Format("delete tbNhaCungCap where id = '{0}'",idNhaCungCap);
            return _conn.ExecuteCMD();
        }
