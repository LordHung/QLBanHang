...
namespace QLBanHang.Models
{
    class mdlLoaiSanPham
    {
        ...
        public DataTable GetData()
        {
            ...
            _conn.CMD.CommandText = "select  tbLoaiSanPham.id,MaNganhSanPham + '/' + TenNganhSanPham as NganhSanPham ,MaLoaiSanPham,TenLoaiSanPham,tbLoaiSanPham.MoTa " + 
        ...
        {
            DataTable table = new DataTable();//Create template table to get data from database
            _conn.CMD.CommandText = "select  id,MaLoaiSanPham + '/' + TenLoaiSanPham as LoaiSanPham from tbLoaiSanPham";
            _conn.FillData(table);
            return table;
        }
        ...
        {
            _conn.CMD.CommandText = "delete tbLoaiSanPham where id = '" + idLoaiSanPham + "'";
            return _conn.ExecuteCMD();
        }

        public int GetIdLoaiSanPhamByMa(string maLoaiSanPham)
        {
            DataTable table = new DataTable();//Create template table to get data from database
            _conn.CMD.CommandText = String.Format("select ISNULL(id,0) as id from tbLoaiSanPham where [tbLoaiSanPham].[MaLoaiSanPham] = '{0}'", maLoaiSanPham);
            _conn.FillData(table);
            return Convert.ToInt32(table.Rows[0]["id"].ToString());
        }

        public string GetMaLoaiSanPhamById(int idMaLoaiSanPham)
        {
            DataTable table = new DataTable();//Create template table to get data from database
            _conn.CMD.CommandText = String.Format("select MaLoaiSanPham from tbLoaiSanPham where [tbLoaiSanPham].[id] = '{0}'", idMaLoaiSanPham);
            _conn.FillData(table);
            return table.Rows[0]["MaLoaiSanPham"].ToString();
        }
    }
}
